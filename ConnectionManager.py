...
class ConnectionManager:
    ...
    def __init__(self, type = "CLIENT") -> None:
        ...
        if type == "RIOT":
            self.baseUrl = f'https://127.0.0.1:{self.riotclient_port}'
            self.session = requests.Session()
            self.session.auth = ('riot', self.riotclient_auth_token)
        else:
    ...
    def _extractConnectionSettings(self) -> None:
        for item in self.proc.cmdline():
            ...
            elif item.startswith('--riotclient-auth-token='):
                self.riotclient_auth_token = item.split('=', 1)[1]
            elif item.startswith('--riotclient-app-port='):
                self.riotclient_port = item.split('=', 1)[1]